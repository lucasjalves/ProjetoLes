...
public class ProcessarRegraNegocioHelper {
	...
	public List<Mensagem> processarRegraNegocio(Object object, String operacao)
	{
		...
		classesRegrasNegocio.forEach(classe -> {
			...
			{
				if(anotacaoRegraNegocio.classe().getCanonicalName().equals(object.getClass().getCanonicalName()) &&
						operacao.equals(anotacaoRegraNegocio.operacao()))
				{
					try {
						Strategy instanciaRegraNegocio = (Strategy) classe.newInstance();
						mensagensRegraNegocio.addAll(instanciaRegraNegocio.processar(object));
